...
int main(int argc, char *argv[])
{
     ...
     do
     {
          ...
          if (!poll_err)
               continue; // If poll timed out we jump back to poll()

          // One or more file descriptors are ready
          current_size = nfds;
          for (int i = 0; i < current_size && poll_err > 0; i++) // Linear cycle over file descriptors, poll_err is the number of descriptors which raised an event so we stop cycling as soon as we checked them all
          {
